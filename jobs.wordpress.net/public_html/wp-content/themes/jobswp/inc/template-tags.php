...
function jobswp_content_nav( $nav_id ) {
	...
	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>
		...
		<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older jobs', 'jobswp' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( __( 'Newer jobs <span class="meta-nav">&rarr;</span>', 'jobswp' ) ); ?></div>
		<?php endif; ?>
 ...
 */
function jobswp_categorized_blog() {
	...
	if ( '1' != $all_the_cool_cats ) {
		// This blog has more than 1 category so jobswp_categorized_blog should return true
		return true;
	} else {
		// This blog has only 1 category so jobswp_categorized_blog should return false
		return false;
	}
}

/**
 * Flush out the transients used in jobswp_categorized_blog
 */
function jobswp_category_transient_flusher() {
	// Like, beat it. Dig?
