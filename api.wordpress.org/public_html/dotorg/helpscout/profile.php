...
if ( isset ( $request->customer->email ) ) {
	...
	// look up profile url for email
	$user = get_user_by( 'email', $request->customer->email );
	if ( isset( $user->user_nicename ) ) {
		$profilelink = '<a href="https://profiles.wordpress.org/' . $user->user_nicename . '">'. $user->user_nicename .'</a>';
	} else {
		$profilelink = 'No profile found';
	}

}

// response to HS is just HTML to display in the sidebar
$response = array ('html' => $profilelink);
