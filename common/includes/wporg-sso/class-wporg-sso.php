	...
	class WPOrg_SSO {
		...
		public function __construct() {
			...
			if ( ! empty( $_SERVER['HTTP_HOST'] ) ) {
				...
				$this->script = esc_url_raw( wp_unslash( $_SERVER['SCRIPT_NAME'] ?? '' ) );
			}
		}
		...
		*/
		...
		public function login_url( $login_url = '', $redirect_to = '' ) {
			...
			if ( empty( $redirect_to ) ) {
				$redirect_to = 'https://' . $this->host . esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ?? '' ) );
			}
		...
		protected function _get_safer_redirect_to( $default = 'https://wordpress.org/' ) {
			...
			if ( ! empty( $_REQUEST['redirect_to'] ) && is_string( $_REQUEST['redirect_to'] ) ) {
				...
				$redirect_to_requested = str_replace( ' ', '%20', esc_url_raw( wp_unslash( $_REQUEST['redirect_to'] ) ) ); // Encode spaces.
				...
				$redirect_to_referrer = esc_url_raw( wp_unslash( $_SERVER['HTTP_REFERER'] ?? '' ) );
