...
function wporg_themes_map_meta_cap( $caps, $cap, $user_id, $context ) {
	switch ( $cap ) {
		...
		case 'theme_admin_edit':
			// Admin-edit actions on a theme post (force-release, etc). Maps onto the
			// same 'suspend_themes' primitive the theme-admin roles already carry;
			// the meta cap exists so check sites can read as "this is an admin edit"
			// rather than overloading the suspend/reinstate semantics. Theme review
			// (the workflow on Trac) is a separate concern and may diverge later.
			$caps[] = 'suspend_themes';
			unset( $caps[ array_search( $cap, $caps ) ] );
			break;
...
function wporg_themes_post_submitbox_misc_actions() {
	...
	elseif ( current_user_can( 'reinstate_theme', $post->ID ) && 'suspend' == $post->post_status ) {
		$links[] = sprintf( '<a class="submit-reinstate_theme" title="%1$s" href="%2$s">%3$s</a>', esc_attr__( 'Reinstate this item', 'wporg-themes' ), esc_url( wporg_themes_get_reinstate_url( $post ) ), __( 'Reinstate', 'wporg-themes' ) );
	}
...
function wporg_themes_meta_box_cooldown_section( $post, $versions ) {
	...
	<?php if ( current_user_can( 'theme_admin_edit', $post->ID ) ) : ?>
		<p>
...
function wporg_themes_save_meta_box_data( $post_id ) {
	...
	if (
		...
		current_user_can( 'theme_admin_edit', $post_id )
	) {
