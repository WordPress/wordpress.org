		...
		<p><?php echo $text; ?> -
			<select name="wporg_themes_status[<?php echo base64_encode( $version ); // base64 because version numbers don't work so well as parts of keys ?>]">
				...
				<?php if ( 'approved' === $status ) : ?>
					<?php // `approved` is a transient Trac-driven cooldown state; only shown so the current value displays correctly. ?>
					<option value="approved" selected><?php esc_html_e( 'Approved (in release cooldown)', 'wporg-themes' ); ?></option>
				<?php endif; ?>
