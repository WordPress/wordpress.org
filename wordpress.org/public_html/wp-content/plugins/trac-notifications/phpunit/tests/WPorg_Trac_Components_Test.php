...
class WPorg_Trac_Components_Test extends WPorg_Trac_Components_TestCase {
	...
	/**
	 * An editor who is also listed keeps the normal form, author field and all.
	 */
	public function test_saving_is_left_alone_for_an_editor_who_is_also_a_maintainer(): void {
		update_post_meta( $this->component, '_active_maintainers', $this->login( $this->maintainer ) . ', ' . $this->login( $this->editor ) );
		wp_set_current_user( $this->editor );
		$this->submit_edit( $this->component, array( 'post_author_override' => $this->bystander ) );

		$this->assertArrayHasKey( 'post_author', $_POST );

		edit_post();

		$this->assertSame( $this->bystander, (int) get_post( $this->component )->post_author, 'An editor can still reassign the page.' );
	}
