<?php
/**
 * This is 99.99% the code from the `vendor/google/cloud-storage/src/StreamWrapper.php`
 * source file, but with a workaround for its non-implementation of
 * `stream_metadata()`. This non-implementation causes use of `chgrp()`, `chmod()`,
 * `chown()`, or `touch()` on a streamed resource to throw a PHP warning.
 *
 * Because PHP doesn't allow existing classes to be amended, and because extending
 * won't work due to the need to access private variables, the entire class is
 * duplicated below but with these minor changes:
 *
 * 1. Added `stream_metadata()` to return false to reflect the lack of support
 *    for operations such as `chgrp()`, `chmod()`, `chown()`, or `touch()`.
 *    Calling any of those functions without the stream srapper implementing
...
class WPORG_StreamWrapper
{
    ...
    public static function register(StorageClient $client, $protocol = null)
    {
        ...
        if (!in_array($protocol, stream_get_wrappers())) {
            if (!stream_wrapper_register($protocol, WPORG_StreamWrapper::class, STREAM_IS_URL)) {
	...
	/**
	 * This wrapper does not support `chgrp()`, `chmod()`, `chown()`, or `touch()`.
	 *
