...
<form class="translation-event-form" action="" method="post">
	...
	<div>
		...
		<input type="text" id="event-title" name="event_title" value="<?php echo esc_attr( $event->title() ); ?>" <?php echo ( $is_create_form || current_user_can( 'edit_translation_event_title', $event->id() ) ) ? '' : 'readonly'; ?> required size="42">
	</div>
	...
	<div>
		...
		<textarea id="event-description" name="event_description" rows="4" cols="40" required <?php echo ( $is_create_form || current_user_can( 'edit_translation_event_description', $event->id() ) ) ? '' : 'readonly'; ?>><?php echo esc_html( $event->description() ); ?></textarea>
		<?php
		echo wp_kses(
		...
		<div>
			...
			<input type="datetime-local" id="event-start" name="event_start" value="<?php echo esc_attr( $event->start()->setTimezone( $event->timezone() )->format( 'Y-m-d H:i' ) ); ?>" required <?php echo ( $is_create_form || current_user_can( 'edit_translation_event_start', $event->id() ) ) ? '' : 'readonly'; ?> >
		</div>
		<div>
			<label for="event-end"><?php esc_html_e( 'End Date', 'gp-translation-events' ); ?></label>
			<input type="datetime-local" id="event-end" name="event_end" value="<?php echo esc_attr( $event->end()->setTimezone( $event->timezone() )->format( 'Y-m-d H:i' ) ); ?>" required <?php echo ( $is_create_form || current_user_can( 'edit_translation_event_end', $event->id() ) ) ? '' : 'readonly'; ?>>
		</div>
		<div>
			<label for="event-timezone"><?php esc_html_e( 'Event Timezone', 'gp-translation-events' ); ?></label>
			<select id="event-timezone" name="event_timezone" required <?php echo ( $is_create_form || current_user_can( 'edit_translation_event_timezone', $event->id() ) ) ? '' : 'disabled'; ?> >
				<?php
				echo wp_kses(
