				...
				<section class="col-8">
					<p><?php
						printf(
							...
							__( 'Learn more about WordPress core software security in this free white paper. You can also download it in <a href="%s">PDF format</a>.', 'wporg' ),
...
using a CMS.', 'wporg' ),
					...
					<h3><?php _e( 'The WordPress Core Leadership Team', 'wporg' ); ?></h3>
					...
					<p><?php
						printf(
							...
							__( 'A release cycle follows the following pattern%s:', 'wporg' ),
					...
					<h3><?php _e( 'Version Numbering and Security Releases', 'wporg' ); ?></h3>
					<p><?php _e( "A major WordPress version is dictated by the first two sequences. For example, 3.5 is a major release, as is 3.6, 3.7, or 4.0. There isn't a &ldquo;WordPress 3&rdquo; or &ldquo;WordPress 4&rdquo; and each major release is referred to by its numbering, e.g., &ldquo;WordPress 3.9.&rdquo;", 'wporg' ); ?></p>
					...
					<p><?php
						printf(
							/* translators: %s: Footnote */
							__( 'A minor WordPress version is dictated by the third sequence. Version 3.5.1 is a minor release, as is 3.4.2%s. A minor release is reserved for fixing security vulnerabilities and addressing critical bugs only. Since new versions of WordPress are released so frequently &mdash; the aim is every 4-5 months for a major release, and minor releases happen as needed &mdash; there is only a need for major and minor releases.', 'wporg' ),
							'<sup id="ref3"><a href="#footnote3">3</a></sup>'
						);
					?></p>

					<h3><?php _e( 'Version Backwards Compatibility', 'wporg' ); ?></h3>
					...
					<p><?php
						printf(
							/* translators: %s: Footnote */
							__( 'If necessary, the core team may decide to fork or replace critical external components, such as when the SWFUpload library was officially replaced by the Plupload library in 3.5.2, and a secure fork of SWFUpload was made available by the security team<%s for those plugins who continued to use SWFUpload in the short-term.', 'wporg' ),
							'sup id="ref15"><a href="#footnote15">15</a></sup>'
						);
					?></p>

					<h4><?php _e( 'A10 - Unvalidated Redirects and Forwards', 'wporg' ); ?></h4>
					<p><?php
						printf(
							/* translators: %s: Footnote */
							__( "WordPress' internal access control and authentication system will protect against attempts to direct users to unwanted destinations or automatic redirects. This functionality is also made available to plugin developers via an API, <code>wp_safe_redirect()</code>%s.", 'wporg' ),
							'<sup id="ref16"><a href="#footnote16">16</a></sup>'
						);
					?></p>
					<h3><?php _e( 'Further Security Risks and Concerns', 'wporg' ); ?></h3>
					...
					<h3><?php _e( 'WordPress.org Theme and Plugin Repositories', 'wporg' ); ?></h3>

					<p><?php
						printf(
							/* translators: 1: Number of plugins - 50,000; 2: Number of themes - 5,000 */
							__( 'There are approximately %1$s+ plugins and %2$s+ themes listed on the WordPress.org site. These themes and plugins are submitted for inclusion and are manually reviewed by volunteers before making them available on the repository.', 'wporg'
),
					...
					<p><?php
						printf(
							...
							__( 'Inclusion of plugins and themes in the repository is not a guarantee that they are free from security vulnerabilities. Guidelines are provided for plugin authors to consult prior to submission for inclusion in the repository%1$s, and extensive documentation about how to do WordPress theme development%2$s is provided on the WordPress.org site.', 'wporg' ),
					...
					<h3><?php _e( 'Database API', 'wporg' ); ?></h3>

					<p><?php
						printf(
							/* translators: %s: Footnote */
							__( 'The Database API%s, added in WordPress 0.71, provides the correct method for accessing data as named values which are stored in the database layer.', 'wporg' ),
							'<sup id="ref24"><a href="#footnote24">24</a></sup>'
						);
					?></p>

					<h3><?php _e( 'Filesystem API', 'wporg' ); ?></h3>

					<p><?php
						printf(
							/* translators: 1: Footnote; 2: Footnote */
							__( 'The Filesystem API%1$s, added in WordPress 2.6%2$s, was originally created for WordPress&lsquo; own automatic updates feature. The Filesystem API abstracts out the functionality needed for reading and writing local files to the filesystem to be done securely, on a variety of host types.', 'wporg' ),
					...
					<h3><?php _e( 'HTTP API', 'wporg' ); ?></h3>

					<p><?php
						printf(
							/* translators: 1: Footnote; 2: Footnote */
							__( 'The HTTP API%1$s, added in WordPress 2.7%2$s and extended further in WordPress 2.8, standardizes the HTTP requests for WordPress. The API handles cookies, gzip encoding and decoding, chunk decoding (if HTTP 1.1), and various other HTTP protocol implementations. The API standardizes requests, tests each method prior to sending, and, based on your server configuration, uses the appropriate method to make the request.', 'wporg' ),
							'<sup id="ref27"><a href="#footnote27">27</a></sup>',
							'<sup id="ref28"><a href="#footnote28">28</a></sup>'
						);
					?></p>

					<h3><?php _e( 'Permissions and current user API', 'wporg' ); ?></h3>

					<p><?php
						printf(
							/* translators: %s: Footnote */
							__( "The permissions and current user API%s is a set of functions which will help verify the current user's permissions and authority to perform any task or operation being requested, and can protect further against unauthorized users accessing or performing functions beyond their permitted capabilities.", 'wporg' ),
							'<sup id="ref29"><a href="#footnote29">29</a></sup>'
						);
					?></p>
					<h3><?php _e( 'White paper content License', 'wporg' ); ?></h3>
					<p><?php
						printf(
							/* translators: 1: Link to WordPress Foundation Trademark Polocy (English); 2: Link to Creative Commons CC0 license (English) */
							__( 'The text in this document (not including the WordPress logo or <a href="%1$s">trademark</a>) is licensed under <a href="%2$s">CC0 1.0 Universal (CC0 1.0) Public Domain Dedication</a>. You can copy, modify, distribute and perform the work, even for commercial purposes, all without asking permission.', 'wporg' ),
					...
					<p><?php
						printf(
							...
							__( "<em>A special thank you to Drupal's </em><a href='%s'><em>security white paper</em></a><em>, which provided some inspiration. </em>", 'wporg' ),
							'http://drupalsecurityreport.org/'
						);
					?></p>
					<h3><?php _e( 'Additional Reading', 'wporg' ); ?></h3>
					<ul>
						<li><?php
							printf(
								/* translators: %s: Link to News Blog including the <a> tags. */
								__( 'WordPress News %s', 'wporg' ),
						...
						<li><?php
							printf(
								...
								__( 'WordPress Security releases %s', 'wporg' ),
						...
						<li><?php
							printf(
								...
								__( 'WordPress Developer Resources %s', 'wporg' ),
					...
					<p><?php _e( '<em>Contributions from</em> Barry Abrahamson, Michael Adams, Jon Cave, Helen Hou-Sand&iacute;, Dion Hulse, Mo Jangda, Paul Maiorana', 'wporg' ); ?></p>
