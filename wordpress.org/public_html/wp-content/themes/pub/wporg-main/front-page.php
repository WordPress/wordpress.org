...
function get_latest_devblog_post() {
	$cached = wp_cache_get('feed', 'wporg');
	if (is_array($cached)) {
		echo '<!-- feed is cached -->';
		return $cached;
	}
	$feed = file_get_contents('http://wordpress.org/news/feed/');
	preg_match('|<item>\s*<title>(.*?)</title>.*?<link>(.*?)</link>.*?<description><!\[CDATA\[(.*?)\]\]>|s', $feed, $matches);
	$matches = array_map('trim', $matches);
	$matches[3] = preg_replace('|\s*\[\.\.\.\]$|', '&hellip;', $matches[3]);
	$latest_post = array('title' => $matches[1], 'link' => $matches[2], 'content' => $matches[3]);
	wp_cache_set('feed', $latest_post, 'wporg', 3600);
	return $latest_post;
}

$latest_post = [];//get_latest_devblog_post();

$headadd = '<link rel="alternate" type="application/rss+xml" title="WordPress Blog RSS" href="http://wordpress.org/news/feed/" />';
$pagetitle = 'Blog Tool, Publishing Platform, and CMS';
$titleover = 'Welcome to WordPress';
...
require WPORGPATH . 'header.php';
 ?>
	...
	<main id="main" class="site-main home-welcome" role="main">
		...
		<div id="home-below">
			<div class="wrapper">
				...
				<div class="col-4">
					...
					<h5><a href="<?php echo $latest_post['link']; ?>"><?php echo $latest_post['title']; ?></a></h5>
					<p><?php echo $latest_post['content']; ?></p>
				</div>
