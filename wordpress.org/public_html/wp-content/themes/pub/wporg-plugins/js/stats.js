...
( function( $, settings ) {
	$( function () {
		...
		$.getJSON('https://api.wordpress.org/stats/plugin/1.0/active-installs.php?slug=' + settings.slug + '&limit=267&callback=?', function( downloads ) {
			google.charts.setOnLoadCallback( function() {
				var data = new google.visualization.DataTable(),
					count = 0,
					sml;
				...
				data.addColumn( 'date', settings.l10n.date );
				data.addColumn( 'number', settings.l10n.growth );
				...
				sml = data.getNumberOfRows() < 36;

				new google.visualization.LineChart( document.getElementById( 'plugin-growth-stats' ) ).draw( data, {
					colors: ['#253578'],
					legend: { position: 'none' },
					titlePosition: 'in',
					axisTitlesPosition: 'in',
