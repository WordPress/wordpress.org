...
use function WordPressdotorg\Plugin_Directory\Theme\{get_blueprint_url, get_latest_release, get_revision_changeset_link};
...
function get_view_diff_check_item( $post ) {
	...
	$info_text = sprintf(
		...
		__( 'Scan through <a target="_blank" href="%s">your changeset</a> before publishing.', 'wporg-plugins' ),
		esc_url( get_revision_changeset_link( $plugin->post_name, $commits ) )
	);
	$content = get_release_item_content( $label, $info_text );
...
function get_version_number_check_item( $verdict, $value ) {
	...
	$info_text = sprintf(
		...
		__( 'Current version: %s', 'wporg-plugins' ),
		'<code>' . $value . '</code>',
	);
...
function get_tested_up_to_check_item( $verdict, $value, $release_post ) {
	...
	$plugin = get_post( $release_post->post_parent );
	...
	if ( empty( $value ) ) {
		...
		$info_text = sprintf(
			/* translators: %s: URL to the plugin in the Plugin Directory */
			__( 'Tested up to is %1$s. <a target="_blank" href="%2$s">Test it now in Playground</a> and update. ', 'wporg-plugins' ),
			'<code>' . $value . '</code>',
...
>
	...
	// 		</div>
	// 		<!-- /wp:wporg/notice -->',
	// 	esc_html__( 'You need to increment your plugin version number and reload the page.', 'wporg-plugins' )
	// );
