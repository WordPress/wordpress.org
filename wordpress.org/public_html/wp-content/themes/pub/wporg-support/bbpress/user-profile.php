...
<div id="bbp-user-profile" class="bbp-user-profile">
		...
		if ( $is_user_flagged ) {
			if ( $moderator && $moderation_date ) {
				$msg = sprintf(
					...
					esc_html__( 'This user has been flagged by %1$s on %2$s at %3$s.', 'wporg-forums' ),
					sprintf( '<a href="%s">%s</a>', esc_url( home_url( "/users/$moderator/" ) ), $moderator ),
					/* translators: localized date format, see https://secure.php.net/date */
					mysql2date( esc_html__( 'F j, Y', 'wporg-forums' ), $moderation_date ),
					/* translators: localized time format, see https://secure.php.net/date */
					mysql2date( esc_html__( 'g:i a', 'wporg-forums' ), $moderation_date )
				);
			} elseif ( $moderator ) {
				$msg = sprintf(
					/* translators: %s: linked moderator's username */
					esc_html__( 'This user has been flagged by %s.', 'wporg-forums' ),
					sprintf( '<a href="%s">%s</a>', esc_url( home_url( "/users/$moderator/" ) ), $moderator )
				);
			} else {
				$msg = esc_html__( 'This user has been flagged.', 'wporg-forums' );
			}
	...
	?>
		...
		if ( $slack_username && $slack_username !== $user_nicename ) {
			...
			printf(
				esc_html__( '%1$s on WordPress.org, %2$s on <a href="%3$s">Slack</a>', 'wporg-forums' ),
			...
			printf(
				esc_html__( '%1$s on WordPress.org and <a href="%2$s">Slack</a>', 'wporg-forums' ),
				wporg_support_get_wporg_profile_link(),
				'https://make.wordpress.org/chat/'
			);
		} else {
