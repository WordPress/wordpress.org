...
| `npm run format`    | Formats the JavaScript, JSON, and Markdown sources with Prettier (`wp-scripts format`). Stylesheets are not Prettier-formatted — `.prettierignore` excludes `*.css`, since the CSS is hand-maintained and `wp4-rtl.css` is generated from it.             |
